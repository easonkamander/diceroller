...
def parseHierarchy (characters, start=0, stop=None, minHierarchy=Hierarchy.Ternary, outer=None, statement=False):
	...
	if Hierarchy.Symbol >= minHierarchy:
		if len(characters) == 1 and characters[0]['charset'] == 2:
			return (Hierarchy.Symbol, characters[0]['text'], 0, 0)
		else:
			minHierarchy = Hierarchy.Symbol
...
class ExpressionSymbol (Expression):
	def __init__ (self, hierarchy, user):
		super().__init__(hierarchy)
		self.user = user

	def evaluate (self, components = None):
		return userdata[self.user][self.components].evaluate(components)

	def __repr__ (self):
		return str(userdata[self.user][self.components])

	def instance (self):
		return userdata[self.user][self.components].instance()
...
def parseExpression (hierarchy, user):
	if hierarchy[0] == Hierarchy.Symbol:
		return ExpressionSymbol(hierarchy, user)
	elif isinstance(hierarchy[1], Fraction):
		return Expressions[hierarchy[0]](hierarchy)
	else:
		hierarchy[1][:] = [parseExpression(h, user) for h in hierarchy[1]]
		return Expressions[hierarchy[0]](hierarchy)
...
def parse (text, user):
	...
	if len(characters) == 0:
		return (False, [Error('No Characters', '', 0)])
	elif isinstance(characters[0], Error):
		return (False, characters)
	else:
		stmtSymbol, stmtCode = None, None

		hierarchy = parseHierarchy(characters, statement=True)

		if hierarchy[0] == Hierarchy.Assignment:
			stmtSymbol, stmtCode = hierarchy[1][0][1], hierarchy[2]
			if stmtCode == 2:
				if stmtSymbol in userdata[user]:
					del userdata[user][stmtSymbol]
				return (True, None)
			hierarchy = hierarchy[1][1]

		hierarchyErrors = parseHierarchyErrors(hierarchy)
		if len(hierarchyErrors):
			return (False, hierarchyErrors)
		else:
			expression = parseExpression(hierarchy, user)
			instance = expression.instance()
			if stmtCode == 0:
				userdata[user][stmtSymbol] = expression
			elif stmtCode == 1:
				userdata[user][stmtSymbol] = instance
			return (True, [expression, instance])

print(parse('abc = 1', 37))
print(userdata)
print(parse('abc+1', 37))
print(userdata)